...
public class WantChangeDAO {
	...
	private static final String SELECT_CHANGE_BY_OWNERID="SELECT"
			...
			+" FROM vgb.changes AS w JOIN vgb.customers AS c ON w.user = c.id JOIN vgb.customers AS c2 ON w.ownerID = c2.id JOIN vgb.products AS p ON w.productID = p.id  "
			+ "WHERE w.ownerId =? AND w.cheack=0";
				...
				try(
					...
					try(
						...
						while(rs.next()) {
							...
							user.setName(rs.getString("c2.name"));
							w.setUser(user);
